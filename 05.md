# Création de l'application CodeXpress
...
2. Voulez-vous utilisez UX Turbo ? Tapez `n` pour non, puis appuyez sur Entrée.
...
On enchaine aven l'entité `Notification` avec la commande suivante :
...
Notez que nous somme dans un champs de vision à partir de l'entité `Category`. Donc, la relation part de l'entité `Category` vers une autre.
...
Contrairement aux autres entités, les utilisateurs d'une application Symfony se **créer** avec une commande différente. Mais pour les mettres à jour, il faut utiliser la commande `make:entity` avec le nom de l'entité `User`.
